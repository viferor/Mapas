...
// ---- DECLARACIÓN GLOBAL DE FUNCIONES DE CARGA (Para evitar errores de "no definido") ----
...
window.procesarImagenCalles = async function(event) {
    ...
    try {
        ...
        if (todasLasLineas.length === 0) {
            mostrarToast("El OCR no ha detectado texto legible.");
            event.target.value = '';
            return;
        }
...
window.alternarBorrado = function() {
    ...
    if (modoActual !== 'borrar') {
        ...
        btn.innerHTML = '🧹';
        map.dragging.enable(); 
        mostrarToast("🧹 Modo: Borrar elementos (toca para eliminar)");
    } else {
        if (!borradoPreciso) {
            borradoPreciso = true;
            ...
            mostrarToast("🧽 Modo: Borrado preciso (pasa el dedo para recortar)");
        } else {
            borradoPreciso = false;
            modoActual = 'ruta';
            btn.className = 'btn';
...
function cerrarModalBuscar() {
    document.getElementById('modal-buscar').classList.remove('active');
}
...
// ---- PROPIEDADES DE GEOLOCALIZACIÓN Y PROCESADO DE CALLES (RADIO AMPLIADO A 15000) ----

const CORDOBA_CENTRO = L.latLng(37.8882, -4.7794);
const RADIO_MAXIMO_METROS = 15000;
const RADIO_PRIMERA_CALLE_METROS = 15000;
...
async function geocodificarUnaCalleNominatim(nombre, centroReferencia, radioMetros, intentos = 2) {
    let nombreLimpio = nombre.replace(/^(c\/|cl\.|calle)\s*/i, '').replace(/^(avda\.|av\.|avenida)\s*/i, '').replace(/^(pza\.|plaza)\s*/i, '').trim();
...
async function procesarListadoCalles(lineas, event) {
    ...
    if (indicesFallidos.length > 0) {
        ...
        const corregidas = await mostrarModalEdicionOCR(nombresFallidos, {
            ...
            mensaje: `Estas ${nombresFallidos.length} calles no se han podido localizar. Corrígelas y pulsa "Reintentar".`,
            textoBoton: 'Reintentar'
        });

        if (corregidas && corregidas.length > 0) {
            mostrarToast(`Reintentando geocodificar ${corregidas.length} calles...`);
    ...
    if (noReconocidasFinal.length > 0) {
        alert(`⚠️ Estas calles se han descartado:\n\n- ${noReconocidasFinal.join('\n- ')}`);
    }

    if (puntosCoordenadas.length === 0) {
        alert("No se ha podido trazar ninguna ruta. Asegúrate de que el archivo contenga nombres de calles de Córdoba.");
        if (event) event.target.value = '';
        return;
    }
    if (puntosCoordenadas.length === 1) {
        mostrarToast("Solo se ha encontrado la primera calle. Las demás están descartadas.");
    }
    ...
    for (let i = 0; i < puntosCoordenadas.length; i++) {
        ...
        if (ultimoPunto) {
            ...
            if (coordsRuta && coordsRuta.length > 0) {
                ...
                // FALLBACK: Si OSRM falla, dibujamos línea recta entre los dos puntos
    ...
    if (tramosSinRuta.length > 0) {
        alert(`⚠️ El servidor de rutas OSRM falló en estos tramos. Se han dibujado en línea recta para que veas el recorrido completo:\n\n- ${tramosSinRuta.join('\n- ')}`);
        mostrarToast("Algunos tramos son líneas rectas (OSRM bloqueado)");
    } else {
        mostrarToast("¡Ruta a pie procesada con éxito!");
    }
    if (event) event.target.value = '';
}
...
// ---- Funciones de GitHub y Auxiliares ----
...
async function cargarMapaDesdeGithub(fileName) {
    ...
    try {
        ...
        if (!res.ok) {
            if (res.status === 404) {
                alert("No se ha encontrado ese mapa. Espera unos segundos y vuelve a intentarlo.");
            } else {
        ...
        try {
            ...
            alert("El archivo del mapa no tiene un formato válido.");
            return;
        }
        if (!geojson || !Array.isArray(geojson.features)) {
            alert("El archivo del mapa no contiene datos.");
            return;
        }
        const capasAQuitar = [];
...
function cargarScriptExterno(src) {
    return new Promise((resolve, reject) => {
        ...
        script.src = src;
        script.onload = () => resolve();
        script.onerror = () => reject(new Error("No se pudo cargar la librería de OCR."));
        document.head.appendChild(script);
    });
}
